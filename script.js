/* ========================================
   script.js — Tiếng Anh Học Đường
   English for School — Static version (fetch + localStorage)
   ======================================== */

/* -- Cache version -- */
var CACHE_VERSION = 'v202607181';
...
/* ============================================
   ...
   ============================================ */
var G_CLASSES = [];          // Array of class objects with lessons
var G_CONVERSATIONS = [];    // Flat list of lessons for current class
var G_CURRENT_CLASS = null;  // Currently selected class id
var G_PROGRESS = {};
var G_RATINGS = {};
var G_GAME_BESTS = {};
...
function lsSave() {
  ...
  try { localStorage.setItem('luyennoi_classes', JSON.stringify(G_CLASSES)); } catch(e) {}
...
function lsLoad() {
  try {
    ...
    var cls = localStorage.getItem('luyennoi_classes');
    if (cls) G_CLASSES = JSON.parse(cls);
...
function resetData() {
  showConfirmModal(
    ...
    function() {
      ...
      localStorage.removeItem('luyennoi_classes');
      ...
      initData().then(function() { switchView('class'); });
    }
  );
}
...
async function initData() {
  ...
  if (G_CLASSES.length > 0 && storedVer === CACHE_VERSION) {
    return;
  }
  try {
    ...
    for (var ci = 0; ci < classes.length; ci++) {
      ...
      allClasses.push({
        id: cls.id || (ci + 1),
        name: cls.name || 'L\u1EDBp ' + (ci + 1),
        icon: cls.icon || '\uD83D\uDCDA',
        subtitle: cls.subtitle || '',
        gradeLevel: cls.gradeLevel || 'beginner',
        lessons: convs
      });
    }
...
function loadClassLessons(classId) {
  G_CURRENT_CLASS = classId;
  G_CONVERSATIONS = [];
  for (var ci = 0; ci < G_CLASSES.length; ci++) {
    if (G_CLASSES[ci].id === classId) {
      G_CONVERSATIONS = G_CLASSES[ci].lessons || [];
      break;
    }
  }
  lsSave();
...
function getCurrentClassName() {
  for (var ci = 0; ci < G_CLASSES.length; ci++) {
    if (G_CLASSES[ci].id === G_CURRENT_CLASS) {
      return G_CLASSES[ci].name;
    }
  }
  return '';
}

function getCurrentClassIcon() {
  for (var ci = 0; ci < G_CLASSES.length; ci++) {
    if (G_CLASSES[ci].id === G_CURRENT_CLASS) {
      return G_CLASSES[ci].icon;
    }
  }
  return '';
}
...
function getLast7DaysStats() {
  ...
  for (var i = 6; i >= 0; i--) {
    ...
    result.push({ key: key, label: weekday, count: G_DAILY[key] || 0 });
  }
  return result;
}
  ...
  try {
    ...
    if ('mediaSession' in navigator) {
      navigator.mediaSession.metadata = new MediaMetadata({
        title: sentenceText || 'Ti\u1EBFng Anh H\u1ECDc \u0110\u01B0\u1EDDng',
...
function switchView(view, data) {
  ...
  if (view === 'class') {
    var cv = document.getElementById('classView');
    if (cv) { cv.classList.remove('hidden'); void cv.offsetWidth; cv.classList.add('animate-fade-in'); }
    renderClassView();
  } else if (view === 'home') {
    ...
    if (data) { currentConvData = data; playerCurrentIdx = -1; if (playerUtterance) { try { speechSynthesis.cancel(); } catch(e) {} playerUtterance = null; } playerIsPlaying = false; }
    ...
    if (data) currentConvData = data;
    roleplayRole = 'A'; roleplayStep = -1; roleplayScore = 0; roleplayAnswered = false; roleplayListening = false; roleplayStepResult = null; roleplayUserText = '';
...
function renderClassView() {
  ...
  if (!container) return;
  if (G_CLASSES.length === 0) {
    container.innerHTML = '<div class="text-center py-12 text-gray-400"><div class="text-4xl mb-3">📭</div><p>Chưa có dữ liệu lớp học.</p></div>';
    return;
  }
  var html = '';
  var gradeColors = ['grade-low','grade-low','grade-low','grade-mid','grade-mid','grade-mid','grade-high','grade-high','grade-high','grade-high','grade-high','grade-high'];
  var gradeIcons = ['🔤','📚','🎨','🏫','🌟','📝','🌍','🎯','🚀','💡','🌐','🎓'];
  var gradeSubs = ['Bảng chữ cái, chào hỏi','Đồ dùng học tập, cơ thể','Đồ ăn, quần áo, nhà cửa','Lớp học, sở thích','Nghề nghiệp, địa điểm','Trường học, kỳ nghỉ','Khu phố, sức khỏe','Du lịch, giải trí','Môi trường, tương lai','Các vấn đề toàn cầu','Đa dạng văn hóa','Nghề nghiệp tương lai'];
  for (var ci = 0; ci < G_CLASSES.length; ci++) {
    var cls = G_CLASSES[ci];
    var gc = gradeColors[ci] || 'grade-low';
    var icon = cls.icon || gradeIcons[ci] || '📚';
    var sub = cls.subtitle || gradeSubs[ci] || '';
    var lessonCount = cls.lessons ? cls.lessons.length : 0;
    html += '<div class="class-card ' + gc + '" onclick="selectClass(' + cls.id + ')">';
    html += '<span class="class-icon">' + icon + '</span>';
    html += '<div class="class-name">' + escHtml(cls.name) + '</div>';
    html += '<div class="class-subtitle">' + escHtml(sub) + '</div>';
    html += '<div class="text-xs text-gray-400 mt-2">' + lessonCount + ' bài học</div>';
    html += '</div>';
  }
...
/* ============================================
   HOME VIEW (Lesson list for selected class)
   ============================================ */
function renderHomeView() {
  var header = document.getElementById('homeHeader');
  if (header) {
    var className = getCurrentClassName();
    var classIcon = getCurrentClassIcon();
    header.innerHTML = '<button onclick="switchView(\'class\')" class="flex items-center gap-1.5 text-purple-600 font-medium py-1 text-sm"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg> ' + classIcon + ' ' + escHtml(className) + '</button><h2 class="text-lg font-semibold text-gray-800">Bài học</h2>';
  }
  var container = document.getElementById('conversationList');
  if (!container) return;
  if (G_CONVERSATIONS.length === 0) {
    container.innerHTML = '<div class="text-center py-12 text-gray-400"><div class="text-4xl mb-3">📭</div><p>Chưa có bài học nào.</p></div>';
    return;
  }
  var html = '';
...
function renderPracticeView() {
  ...
  html += '<div id="rippleRing" class="ripple-ring" style="display:none"></div></div></div>';
...
function showCompletion() {
  ...
  html += '<p class="text-gray-500 mb-8 text-lg md:text-xl">Bạn đã luyện xong bài <strong class="text-purple-600">' + escHtml(currentConvData.title) + '</strong></p>';
...
/* ============================================
   VOCABULARY GAME — Writing-style (uses vocab data)
   ============================================ */
...
function initVocabGame() {
  ...
  vocabIndex = 0; vocabScore = 0; vocabAnswered = false; vocabHearMode = false;
  if (vocabQuestions.length === 0) { showToast('Bài học này chưa có từ vựng.', 'warning'); switchView('home'); return; }
  renderVocabQuestion();
}
function setVocabMode(hear) { vocabHearMode = hear; renderVocabQuestion(); }
...
function renderVocabQuestion() {
  ...
  html += '<div class="mb-3"><div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden"><div class="progress-fill h-full rounded-full bg-amber-500" style="width:' + pct + '%"></div></div><p class="text-right text-xs text-gray-500 mt-0.5">Câu ' + (vocabIndex + 1) + ' / ' + total + ' · ⭐ ' + vocabScore + '</p></div>';
  html += '<div class="flex justify-center gap-2 mb-3">';
  html += '<button id="vocabModeSee" class="px-4 py-2 rounded-lg text-sm font-medium transition-all ' + (!vocabHearMode ? 'bg-amber-100 text-amber-700 font-semibold shadow-sm' : 'bg-gray-100 text-gray-500') + '" onclick="setVocabMode(false)">👁️ Xem nghĩa</button>';
  html += '<button id="vocabModeHear" class="px-4 py-2 rounded-lg text-sm font-medium transition-all ' + (vocabHearMode ? 'bg-amber-100 text-amber-700 font-semibold shadow-sm' : 'bg-gray-100 text-gray-500') + '" onclick="setVocabMode(true)">🔊 Nghe từ</button>';
  html += '</div>';
  if (!vocabHearMode) {
    html += '<div class="bg-white rounded-card shadow-card p-5 text-center mb-4"><div class="text-3xl mb-2">📝</div><p class="text-xs text-gray-400 uppercase tracking-wide mb-2">Nhập từ/cụm từ tiếng Anh</p><p class="text-gray-500 mb-1">Dịch:</p><p class="font-bold text-gray-900 text-xl">' + escHtml(q.vi) + '</p></div>';
  } else {
    html += '<div class="bg-white rounded-card shadow-card p-5 text-center mb-4"><div class="text-3xl mb-2">🔊</div><p class="text-xs text-gray-400 uppercase tracking-wide mb-2">Nghe và nhập từ tiếng Anh</p><button id="vocabHearBtn" class="w-20 h-20 bg-amber-100 hover:bg-amber-200 text-amber-600 rounded-full text-4xl mx-auto mb-4 flex items-center justify-center transition-all shadow-sm" onclick="playVocabWord()">🔊</button></div>';
  }
  html += '<div class="mb-3"><input id="vocabInput" type="text" class="writing-input" placeholder="Gõ từ tiếng Anh..." onkeydown="if(event.key===\'Enter\') checkVocabAnswer()"></div>';
  html += '<button id="vocabCheckBtn" onclick="checkVocabAnswer()" class="w-full bg-amber-500 text-white font-semibold py-3.5 rounded-button shadow text-lg">✔ Kiểm tra</button>';
  html += '<div id="vocabResult" class="mt-3"></div><div id="vocabNextWrap" class="text-center mt-3" style="display:none"><button onclick="nextVocab()" class="bg-amber-500 text-white font-semibold py-3 px-8 rounded-button shadow">Tiếp theo →</button></div>';
  c.innerHTML = html;
  if (vocabHearMode) { setTimeout(function() { playVocabWord(); }, 500); }
  setTimeout(function() { var inp = document.getElementById('vocabInput'); if (inp) inp.focus(); }, 100);
}
function checkVocabAnswer() {
  ...
  if (!userText) { showToast('Vui lòng nhập từ tiếng Anh', 'warning'); inp.focus(); return; }
  vocabAnswered = true;
  var q = vocabQuestions[vocabIndex];
  var nu = normalizeText(userText); var nc = normalizeText(q.en);
  var isCorrect = (nu === nc);
  if (isCorrect) {
    vocabScore++;
    inp.classList.add('correct-input'); inp.disabled = true;
    document.getElementById('vocabResult').innerHTML = '<div class="result-correct p-3 text-center result-pop"><div class="text-lg">✅ Đúng! +1 ⭐</div></div>';
    document.getElementById('vocabCheckBtn').style.display = 'none';
    playTingSound();
    setTimeout(function() {
      nextVocab();
    ...
    document.getElementById('vocabResult').innerHTML = '<div class="result-incorrect p-3 text-center result-pop"><div class="text-lg mb-1">❌ Sai rồi!</div><p class="text-red-700 font-medium text-sm mb-1">Bạn viết: ' + escHtml(userText) + '</p><p class="text-gray-700 text-sm">Đáp án: <strong>' + escHtml(q.en) + '</strong></p><div class="mt-2 flex justify-center gap-2">' + (vocabHearMode ? '<button onclick="playVocabWord()" class="inline-flex items-center gap-1 py-1.5 px-3 rounded-full bg-amber-100 text-amber-700 text-sm font-medium">🔊 Nghe lại</button>' : '') + '<button onclick="retryVocab()" class="inline-flex items-center gap-1 py-1.5 px-3 rounded-full bg-red-100 text-red-700 text-sm font-medium">✏️ Nhập lại</button></div></div>';
  }
  document.getElementById('vocabCheckBtn').style.display = 'none';
}
function retryVocab() {
  ...
  document.getElementById('vocabNextWrap').style.display = 'none';
  document.getElementById('vocabCheckBtn').style.display = 'block';
}
...
function renderVocabResult() {
  ...
  var html = '<div class="text-center py-8 animate-scale-in"><div class="text-6xl mb-4">🎯</div><h3 class="text-2xl font-bold text-gray-800 mb-1">Kết thúc!</h3><p class="text-gray-500 mb-4">Bạn đã hoàn thành phần Từ vựng</p><div class="inline-flex items-center justify-center w-24 h-24 rounded-full mb-4 ' + rc + '" style="font-size:2rem;font-weight:700">' + vocabScore + '/' + total + '</div><p class="text-gray-400 text-sm mb-6">' + pct + '% đúng</p><div class="flex gap-3 justify-center"><button onclick="switchView(\'home\')" class="bg-white text-gray-700 font-medium py-3 px-6 rounded-button border hover:bg-gray-50">← Về danh sách</button><button onclick="initVocabGame()" class="bg-amber-500 text-white font-semibold py-3 px-6 rounded-button shadow">Chơi lại</button></div></div>';
  c.innerHTML = html;
}
...
function renderPlayerView() {
  ...
  html += '</div></div>';
  var hasVideo = conv.videoUrl && conv.videoUrl.trim();
  html += '<div class="player-control-bar"><button class="player-btn player-btn-play" id="playerPlayBtn" onclick="togglePlayerPlay()">▶</button>';
  if (hasVideo) {
    html += '<button class="player-btn" style="background:var(--red-500);color:white;width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;box-shadow:0 2px 8px rgba(239,68,68,.3);transition:all .15s ease" onclick="openVideoModal()" title="Xem video">📺</button>';
  }
  html += '<span class="player-playing-indicator" id="playerIndicator">Sẵn sàng</span></div>';
  // Video modal — body is empty; iframe created dynamically in openVideoModal
  html += '<div id="playerVideoModal" class="video-modal-overlay" style="display:none" onclick="closeVideoModal(event)">';
  html += '<div class="video-modal-box"><div class="video-modal-header"><span class="font-semibold text-gray-800">📺 Video hướng dẫn</span><button onclick="closeVideoModal()" style="background:none;border:none;font-size:1.5rem;cursor:pointer;color:#6b7280;padding:0;line-height:1">×</button></div>';
  html += '<div class="video-modal-body" id="playerVideoBody"></div></div></div>';
...
/* Video modal for player — dynamic iframe (autoplay on open, destroy on close) */
function openVideoModal() {
  var modal = document.getElementById('playerVideoModal');
  if (!modal) return;
  modal.style.display = 'flex';
  document.body.style.overflow = 'hidden';

  var body = document.getElementById('playerVideoBody');
  if (!body) return;
  body.innerHTML = '';
  var videoUrl = currentConvData && currentConvData.videoUrl ? currentConvData.videoUrl.trim() : '';
  if (!videoUrl) {
    body.innerHTML = '<p class="text-gray-400 text-center py-8">Chưa có video cho bài học này</p>';
    return;
  }
  // Add autoplay+ muted params for browser autoplay policy
  var sep = videoUrl.indexOf('?') >= 0 ? '&' : '?';
  var src = videoUrl + sep + 'autoplay=1&mute=1';
  var wrap = document.createElement('div');
  wrap.className = 'video-responsive';
...
function closeVideoModal(event) {
  ...
  // Destroy iframe to stop video/audio completely
  var body = document.getElementById('playerVideoBody');
...
function playerPlayNext() {
  ...
  if (playerVietnameseFirst && s.vi && s.vi.trim()) {
...
var roleplayStepResult = null;
var roleplayUserText = '';
...
function renderRoleplayView() {
  ...
  if (roleplayStep === -1) {
    ...
    for (var i = 0; i <= displayEnd; i++) {
      ...
      if (showVi) { html += '<p class="text-xs mt-1 opacity-60">' + escHtml(s.vi) + '</p>'; }
    ...
    if (roleplayStep < total && roleplayStep >= 0) {
      if (isUserStep(roleplayStep)) {
        if (!roleplayAnswered && roleplayStepResult === null) {
          ...
          html += '<div class="relative"><div id="rpMicBtn" class="w-24 h-24 rounded-full bg-indigo-500 hover:bg-indigo-600 text-white shadow-lg flex items-center justify-center cursor-pointer transition-all" style="font-size:42px" onclick="roleplayToggleMic()"><span id="rpMicIcon">🎤</span></div><div id="rpRippleRing" class="ripple-ring" style="display:none"></div></div>';
          html += '<p id="rpStatus" class="text-base text-gray-500 font-medium min-h-[24px]">🎤 Đến lượt bạn!</p></div>';
          ...
          if (roleplayUserText) { html += '<p class="text-sm text-red-600">Bạn nói: "' + escHtml(roleplayUserText) + '"</p>'; }
...
function roleplayNext() {
  ...
  var sentences = currentConvData.sentences;
  roleplayStep++; roleplayAnswered = false; roleplayStepResult = null; roleplayUserText = '';
  if (roleplayStep >= sentences.length) { renderRoleplayView(); return; }
  ...
  if (!isUser) {
    setTimeout(function() {
      ...
      var u = new SpeechSynthesisUtterance(s.en); u.lang = 'en-US'; u.rate = 0.85;
      u.onend = function() { setTimeout(function() { roleplayNext(); }, 1200); };
      u.onerror = function() { setTimeout(function() { roleplayNext(); }, 800); };
      speechSynthesis.speak(u);
    }, 200);
  }
}
function roleplayToggleMic() { if (roleplayListening) { roleplayStopListening(); } else { roleplayStartListening(); } }
function roleplayStartListening() {
  ...
  try {
    ...
    nr.onresult = function(ev) {
      got = true; roleplayListening = false; roleplayStopRecognition();
      var t = ev.results[0][0].transcript.trim();
    ...
    nr.onend = function() {
      if (roleplayRecognition === nr && !got && roleplayListening) {
        roleplayListening = false; roleplayResetMicUI();
...
function roleplayStopRecognition() { if (roleplayRecognition) { try { roleplayRecognition.stop(); } catch(e) { try { roleplayRecognition.abort(); } catch(e2) {} } roleplayRecognition = null; } }
function roleplayStopListening() { roleplayListening = false; roleplayStopRecognition(); roleplayResetMicUI(); }
function roleplayResetMicUI() { var btn = document.getElementById('rpMicBtn'), icon = document.getElementById('rpMicIcon'), ring = document.getElementById('rpRippleRing'); if (btn) btn.classList.remove('recording'); if (icon) icon.textContent = '🎤'; if (ring) ring.style.display = 'none'; }
function roleplayEvaluate(text) {
  ...
  var nr = normalizeText(text); var no = normalizeText(s.en);
  var isCorrect = (nr === no); roleplayUserText = text;
  if (isCorrect) { roleplayScore++; roleplayStepResult = 'correct'; playTingSound(); renderRoleplayView(); setTimeout(function() { roleplayNext(); }, 1200); }
  else { roleplayStepResult = 'wrong'; renderRoleplayView(); }
}
function roleplayRetry() { roleplayAnswered = false; roleplayStepResult = null; roleplayUserText = ''; renderRoleplayView(); }
...
function initFlashcardGame() {
  if (!currentConvData || !currentConvData.sentences || !currentConvData.sentences.length) {
    showToast('Bài học này chưa có câu nào', 'warning');
    switchView('home');
    return;
  }
  fcList = [];
  ...
  try { var lrn = localStorage.getItem('luyennoi_fc_' + currentConvData.id); if (lrn) fcLearned = JSON.parse(lrn); else fcLearned = {}; } catch(e) { fcLearned = {}; }
  renderFlashcard();
}
function renderFlashcard() {
  ...
  html += '<div class="flashcard-face flashcard-back"><div class="fc-label">📖 Nghĩa</div><div class="fc-text">' + escHtml(card.vi) + '</div>' + (card.ipa ? '<div class="fc-ipa">🔤 ' + escHtml(card.ipa) + '</div>' : '') + '</div></div></div>';
  ...
  html += '</div><div class="text-center mt-4 text-xs text-gray-400">' + (fcIndex + 1) + ' / ' + total + '</div>';
  c.innerHTML = html;
}
...
function playFlashcardAudio() {
  ...
  try { speechSynthesis.cancel(); } catch(e) {}
  var u = new SpeechSynthesisUtterance(fcList[fcIndex].en); u.lang = 'en-US'; u.rate = 0.85;
  speechSynthesis.speak(u);
}
...
function renderReviewView() {
  ...
  for (var ri = 0; ri < G_CONVERSATIONS.length; ri++) {
    ...
    for (var rj = 0; rj < rc.sentences.length; rj++) {
      ...
      if (corrects < 2) {
        weakSentences.push({ convId: rc.id, convTitle: rc.title, idx: rj, en: rc.sentences[rj].en, vi: rc.sentences[rj].vi || '', corrects: corrects });
      }
    }
  }
  var html = '';
  ...
  if (weakSentences.length === 0) {
    ...
    for (var rk = 0; rk < weakSentences.length; rk++) {
      ...
      html += '<span class="text-gray-300 text-lg">🎤</span></div>';
    }
    html += '</div>';
  }
  c.innerHTML = html;
...
function reviewPractice(convId, idx) {
  for (var ri = 0; ri < G_CONVERSATIONS.length; ri++) {
    if (G_CONVERSATIONS[ri].id == convId) {
      currentConvData = G_CONVERSATIONS[ri]; currentSentenceIndex = idx;
      switchView('practice'); return;
    }
  }
...
function renderStatsView() {
  ...
  var maxCount = 1;
  for (var si = 0; si < stats.length; si++) { if (stats[si].count > maxCount) maxCount = stats[si].count; }
  var totalWeek = 0;
  ...
  html += '<p class="text-xs text-gray-400">Số câu đã đọc đúng mỗi ngày</p></div>';
  ...
  for (var sl = 0; sl < stats.length; sl++) {
    var d = new Date(); d.setDate(d.getDate() - (6 - sl));
    ...
    html += '<span class="font-semibold text-sm ' + (stats[sl].count > 0 ? 'text-green-600' : 'text-gray-400') + '">' + stats[sl].count + ' câu</span></div>';
  }
  html += '</div>';
  c.innerHTML = html;
}
